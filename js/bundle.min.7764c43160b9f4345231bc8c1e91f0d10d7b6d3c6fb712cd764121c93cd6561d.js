...
document.getElementById("close-privacy").addEventListener('click',closePrivacy);function increaseLikeCounter(){var counters=document.querySelectorAll("#loved-count")
...
function submitLike(){var slug=getSlug()
...
params.push(param)});var http=new XMLHttpRequest();http.open("POST","https://ruddra-comments.netlify.app/.netlify/functions/server/v3/entry/github/ruddra/ruddra.likes/master/comments/",true);http.setRequestHeader("Content-type","application/x-www-form-urlencoded");http.setRequestHeader("Access-Control-Allow-Origin","*");http.send(params.join('&'));storeLiked()}
...
listener_mb.addEventListener('click',submitLike);if(window.location.hash=="#newsletter-submitted"){var $comTarget=document.getElementById('newsletter-submitted-box');$comTarget.style.display='block';var $comTarget=document.getElementById('newsletter-body');$comTarget.style.display='none';}else if(window.location.hash=="#newsletter-error"){var $comTarget=document.getElementById('newsletter-error');$comTarget.style.display='block';}
